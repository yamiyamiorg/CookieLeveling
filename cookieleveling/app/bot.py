...
from cookieleveling.tracking.host import (
...
from cookieleveling.services.role_assigner import sync_lifetime_roles
from cookieleveling.services.member_sync import refresh_member_cache, sync_member_state
from cookieleveling.tracking.voice import handle_voice_state_update, restore_voice_state
...
class CookieLevelingBot(discord.Client):
    ...
    async def on_ready(self) -> None:
        ...
        guild = self.get_guild(self.config.guild_id)
        if guild is not None and (not self._rankboard_warmed or not self._hostboard_warmed):
            try:
                await sync_member_state(guild)
            except Exception:
                _LOGGER.exception("initial member_state sync failed")
    ...
    async def on_member_join(self, member: discord.Member) -> None:
        if member.guild.id != self.config.guild_id:
            return
        try:
            await refresh_member_cache(member.guild, [member.id])
        except Exception:
            _LOGGER.exception("member cache update failed on join")
    ...
    async def on_member_remove(self, member: discord.Member) -> None:
        ...
        try:
            ...
            _LOGGER.exception("member cache update failed on remove")
